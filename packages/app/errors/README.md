...
### Checking error types: `isInstance`

Every error class exposes a static `isInstance()` type guard — the supported
way to check errors from this package. Do not use the `instanceof` operator:
...
`isInstance` is **inherited by every subclass** and narrows to the class it is
...
Override it in a subclass only if you need custom matching logic; the default
inherited guard covers the standard cases.

### How `isInstance` stays reliable across realms and package copies

On instantiation each error tags itself with `Symbol.for` symbols derived from
its inheritance path (e.g.
`'@lokalise/errors.EnhancedError.InternalError.TranslatorTimeoutError'`), and
...
- **Same name + same path = same class.** Two classes with the same name and
  inheritance path match each other's `isInstance`. That is deliberate — it is
...
- **Names must survive to runtime.** Minifiers that mangle class names (terser
  ...
  each bundle independently, silently breaking `isInstance` across realms and
