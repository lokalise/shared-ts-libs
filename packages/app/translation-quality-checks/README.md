...
scoring. Pure and dependency-light (same input, same issues, no I/O), so it runs in the browser or
in any backend with identical results.
...
| `DOUBLE_WHITESPACE` | `{ whitespaces }`: every run of 2+ consecutive horizontal whitespace characters found |
