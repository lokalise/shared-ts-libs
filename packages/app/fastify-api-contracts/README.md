...
> **Caveat:** `expectedContentType` is negotiated against the content-types declared by **every** status code of the contract, error responses included — not just the success entry. For a contract with an SSE-only success and a JSON error (`200: { content: { 'text/event-stream': … } }`, `404: z.object(…)`), a client sending `Accept: application/json` gets `expectedContentType: 'application/json'` even though the 200 cannot produce it, and a branch like the one above would silently take the non-streaming path. Treat `expectedContentType` as the client's preference, not as proof that the success status declares that representation. To negotiate against the success entry's content-types only, call the exported `determineResponseContentType(request, contentTypes)` with your own candidate list.
