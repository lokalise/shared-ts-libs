...
- a97b5f4: Propagate the contract's `tags` into the Fastify route schema in `buildFastifyRoute`, `buildFastifyNoPayloadRoute` and `buildFastifyPayloadRoute`, so contracts built with `buildRestContract({ tags: [...] })` produce tagged operations in the generated OpenAPI spec.
