...
describe('defineFastifyRoute', () => {
  describe('GET routes', () => {
    it('registers and handles a GET route', async () => {
      ...
      const response = await app.inject().get('/users/42').end()
    ...
    it('handles query params', async () => {
      ...
      const app = await initApp(route)
      const response = await app.inject().get('/search').query({ q: 'hello' }).end()
  ...
  describe('POST routes', () => {
    it('registers and handles a POST route', async () => {
      ...
      const app = await initApp(route)
      const response = await app.inject().post('/users').body({ name: 'Alice' }).end()
  ...
  describe('DELETE routes', () => {
    it('registers and handles a DELETE route with ContractNoBody', async () => {
      ...
      const app = await initApp(route)
      const response = await app.inject().delete('/users/99').end()
  ...
  describe('PATCH routes', () => {
    it('registers and handles a PATCH route', async () => {
      ...
      const app = await initApp(route)
      const response = await app.inject().patch('/users/5').body({ name: 'Bob' }).end()
