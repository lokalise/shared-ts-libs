...
describe('buildFastifyApiRoute — SSE-only', () => {
  it('produces a route with sse: true', () => {
    const routeOptions = buildFastifyApiRoute(sseOnlyContract, (_request, sse) => {
      sse.start('keepAlive')
    })
...
describe('buildFastifyApiRoute — dual-mode', () => {
  it('produces a route with sse: true', () => {
    const routeOptions = buildFastifyApiRoute(dualModeContract, {
...
describe('buildFastifyApiRoute — response schemas', () => {
  ...
  it('picks the JSON schema from anyOfResponses even when SSE variant comes first', () => {
    ...
    const routeOptions = buildFastifyApiRoute(sseFirstContract, {
      nonSse: async () => ({ status: 200, body: { id: '1', name: 'Alice' } }),
      sse: (_request, sse) => {
        sse.start('keepAlive')
      },
...
describe('buildFastifyApiRoute — runtime', () => {
  ...
  it('streams events from an SSE-only autoClose handler', async () => {
    app = await buildApp()
    app.route(
      buildFastifyApiRoute(sseOnlyContract, async (_request, sse) => {
  ...
  it('returns the early HTTP response from sse.respond() before streaming', async () => {
    app = await buildApp()
    app.route(
      buildFastifyApiRoute(sseOnlyContract, (_request, sse) => {
        sse.respond(503, { error: 'unavailable' })
      }),
    )
    await app.ready()
  ...
  it('branches a dual-mode route to JSON when Accept is application/json', async () => {
    app = await buildApp()
    app.route(
      buildFastifyApiRoute(dualModeContract, {
        nonSse: (request) => ({
          status: 200,
  ...
  it('branches a dual-mode route to SSE when Accept is text/event-stream', async () => {
    app = await buildApp()
    app.route(
      buildFastifyApiRoute(dualModeContract, {
        nonSse: () => ({ status: 200, body: { id: '1', name: 'sync' } }),
        sse: async (_request, sse) => {
          const session = sse.start('autoClose')
