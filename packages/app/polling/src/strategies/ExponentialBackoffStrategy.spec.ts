...
describe("ExponentialBackoffStrategy", () => {
	...
	describe("successful polling", () => {
		it("should complete immediately if first attempt succeeds", async () => {
			...
			let attemptCount = 0;
			const result = await strategy.execute<string>(async (attempt) => {
		...
		it("should retry and complete on third attempt", async () => {
			...
			const result = await strategy.execute<string>(async (attempt) => {
				...
				return { isComplete: true, value: `completed-after-${attempt}` };
			}, createTestContext());
		...
		it("should complete just before timeout", async () => {
			...
			const result = await strategy.execute<number>(async (attempt) => {
				attempts.push(attempt);
				if (attempt === 5) {
					return { isComplete: true, value: 42 };
				}
				return { isComplete: false };
			}, createTestContext());
		...
		it("should pass attempt number correctly", async () => {
			...
			await strategy.execute<void>(async (attempt) => {
				receivedAttempts.push(attempt);
				if (attempt === 3) {
					return { isComplete: true, value: undefined };
				}
				return { isComplete: false };
			}, createTestContext());
	...
	describe("error propagation", () => {
		it("should propagate errors thrown by pollFn", async () => {
			...
			await expect(
				strategy.execute<string>(async (attempt) => {
					if (attempt === 2) {
	...
	describe("delay calculation", () => {
		it("should respect maxDelayMs cap even with jitter", async () => {
			...
			try {
				await strategy.execute<string>(async (attempt) => {
					attempts.push(attempt);
					const now = Date.now();
		...
		it("should use fixed interval when backoffMultiplier is 1", async () => {
			...
			try {
				await strategy.execute<string>(async () => {
	...
	describe("standard configuration", () => {
		...
		it("should work with standard configuration", async () => {
			...
			const result = await strategy.execute<number>(async (attempt) => {
				if (attempt === 2) {
					return { isComplete: true, value: 100 };
				}
				return { isComplete: false };
			}, createTestContext());
