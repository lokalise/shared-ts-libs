...
import type { QueueConfiguration } from '../managers/AbstractQueueManager.js'
import { FakeUpdatedBackgroundJobProcessor } from './FakeUpdatedBackgroundJobProcessor.js'
import {
    TestSuccessUpdatedBackgroundJobProcessor
} from '../../../test/processors/TestSuccessUpdatedBackgroundJobProcessor.js'
import {
    TestFailingUpdatedBackgroundJobProcessor
} from '../../../test/processors/TestFailingUpdatedBackgroundJobProcessor.js'
import {
    TestStalledUpdatedBackgroundJobProcessor
} from '../../../test/processors/TestStalledUpdatedBackgroundJobProcessor.js'
...
describe('AbstractBackgroundJobProcessor', () => {
    ...
    describe('start', () => {
        ...
        it('throws an error if queue id is not unique', async () => {
            const job1 = new FakeUpdatedBackgroundJobProcessor<JobData>(deps, 'queue1', mocks.getRedisConfig())
            const job2 = new FakeUpdatedBackgroundJobProcessor<JobData>(deps, 'queue2', mocks.getRedisConfig())

            await job1.start()
            await job2.start()
            await expect(
                new FakeUpdatedBackgroundJobProcessor<JobData>(deps, 'queue1', mocks.getRedisConfig()).start(),
