...
Analysis date: 2026-09-10. Repositories inspected: `maestro`, `content-type-app-engine` (CTENG), `shared-ts-libs`. pg-boss facts checked against [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898), released as pg-boss 12.31.0 on 2026-09-10 and now the registry's `latest`, which is the code this design is written against.
...
Recommendation for decision 1: **`rrule-temporal` behind a library-agnostic API**, declared on the same `^2.2.4` range pg-boss declares so the library and the backend cannot drift onto two copies of the same engine, with the fixture corpus as the swap guarantee.
...
- A deployment-wide floor of pg-boss 12.31.0. An instance still on 12.30.0 reads a stored rule as cron, warns `invalid_schedule` and skips the row, so the bump has to reach every instance before the first rule schedule is written.
...
Checked against released 12.31.0, which is [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898). Only what shapes the design is listed.
...
[timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898) merged on 2026-09-10 (27 files, migration v41, tests and docs) and shipped the same day as 12.31.0, which is the registry's `latest`. What our deployments run today is 12.30.0: `schedule()` takes a cron string and nothing else, the timekeeper evaluates `cron-parser`'s `prev()` against database time every 30 seconds, sends when the occurrence is inside the last 60 seconds, and deduplicates across instances with `singletonSeconds: 60`.

What the release changes:

- **RRULE is a built-in expression format** rather than a plugin point. A rule goes in the same string argument a cron expression goes in, and an expression carrying a `FREQ=` part or opening with an iCalendar property is read as one. Occurrences come from `rrule-temporal`, which pg-boss now depends on at `^2.2.4`.
...
The version gate is deployment-wide, not per-process, so the bump to 12.31.0 reaches every instance of a deployment before the first rule schedule is written there:
...
- The pg-boss adapter is built last (step 5 of the migration outline). 12.31.0 is released, so the adapter is a mapping onto `boss.schedule` and there is no fallback strategy to write.
- Recurrence is pg-boss's own, gated on the whole deployment running 12.31.0 or newer. One adapter covers both expression formats, which differ only in what goes in the expression argument.
...
- `@lokalise/scheduling-common/pg-boss`: schedule registration and reads, queue setup (`short` policy), worker registration, transactional send helper, the fan-out and grouping of a multi-rule recurrence. `pg-boss` as an optional peer dependency, `>=12.31.0`.
...
Dependencies of core: `zod`, `rrule-temporal` on the same `^2.2.4` range pg-boss declares, `cron-parser` for legacy cron patterns. No dependency on `background-jobs-common`, `ioredis`, `bullmq` or `pg-boss` in core. Node 22.12 or newer, which is pg-boss's own floor and `rrule-temporal`'s. The conformance suite runs against both entry points, so the package's docker-compose needs a Postgres container alongside the Redis one `background-jobs-common` already uses.
...
5. **pg-boss adapter.** `boss.schedule` per rule with `missed` in the options, gated on the bump to 12.31.0 having rolled out everywhere, per "Rolling out the upgrade". Full test suite (upsert reschedule, group fan-out and tail deletion, retention, transactional send, reconcile, both `missed` policies in both expression formats, occurrence resolution on the fire path). The conformance suite starts running against both adapters here. The remaining upstream proposals get filed alongside, occurrence identity on a fired job first.
...
- **Engine choice.** Recommended: `rrule-temporal`, the engine pg-boss uses from 12.31.0, on the same `^2.2.4` range pg-boss declares (decision 1). It is pure JS with one types-only dependency and needs no polyfill, and choosing it means the library and the backend cannot disagree about when a rule fires. What it costs is maestro giving up `rrule-rust`, which has more of our own production mileage than the replacement does, and a young dependency at the centre of two products' scheduling. The shared range, the fixture corpus and step 1's side-by-side run are the three things that make that acceptable, and none of them is optional.
...
- **The 12.31.0 floor is deployment-wide.** [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898) is released, so the expression grammar, the `kind` semantics, the `previewSchedule` signature and the `missed` policy are settled rather than provisional, and this document is written against shipped code. What is left is the rollout: an instance still on 12.30.0 reads a stored rule as a cron expression, warns `invalid_schedule` and skips the row, so the bump reaches every instance of a deployment before its first rule schedule is written. That is step 5's first task, per "Rolling out the upgrade".
