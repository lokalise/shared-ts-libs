...
Analysis date: 2026-09-14. Repositories inspected: `maestro`, `content-type-app-engine` (CTENG), `shared-ts-libs`. pg-boss facts checked against released 12.32.0, the registry's `latest` and the code this design is written against: [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898) shipped rule schedules in 12.31.0 on 2026-09-10, and [timgit/pg-boss#890](https://github.com/timgit/pg-boss/pull/890) shipped transactional workers in 12.32.0 on 2026-09-14.
...
- **pg-boss's `schedule()` takes an RRULE from 12.31.0, which is released.** [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898) makes RRULE a built-in expression format rather than a plugin point: a rule goes in the same string argument a cron expression goes in, `schedule()` decides which format it is and records the answer in a new `kind` column, and occurrences come from `rrule-temporal`. It also adds `getSchedule(name, key)`, a synchronous `previewSchedule(expression, { tz, from, count })`, `lastJobId`, and a `missed: 'skip' | 'once'` catch-up policy. The adapter is then a direct mapping onto `boss.schedule`, `boss.unschedule`, `boss.getSchedule` and `previewSchedule`, with the schedule row as the runtime registry. Nothing in the plan waits on upstream any more; what is left for the adapter is the dependency bump and its rollout, since an instance still on 12.30.0 reads a stored rule as cron and skips the row.
- **A fire handler commits with the job's completion from 12.32.0.** [timgit/pg-boss#890](https://github.com/timgit/pg-boss/pull/890) adds `work(name, { transactional: true }, (jobs, tx) => ...)`: pg-boss opens a transaction around the handler and completes the job inside it, so the domain rows a fire writes, the jobs it enqueues through `{ db: tx }` and the record that the occurrence was handled all land together or not at all. That is what `ctx.db` is on the pg-boss adapter, and it closes the case where the work committed and the acknowledgement did not. It does not make an occurrence exactly one job, so `ctx.fireId` stays on the surface: it is still what a handler keys on when a pass sends two jobs for one occurrence, it is all BullMQ can offer, and a handler's HTTP calls sit outside any transaction regardless. The price is a pool connection held for the handler's duration and a snapshot that holds vacuum back while it runs, both of which suit a handler that enqueues and returns, which the 24-hour `active` cap already pushed this design towards.
...
| `rrule-temporal` 2.2.5 (pure JS) | RRULE on top of the `Temporal` API. Correct zoned arithmetic falls out of `Temporal.ZonedDateTime`. One rule per instance, no set type, and a `strict` mode that rejects the combinations RFC 5545 forbids outright. | Strong: DST and calendar math are Temporal's job. | Pure JS, one dependency, and that one (`temporal-spec`) is types only. Needs no polyfill installed: the dist bundles fullcalendar's `temporal-polyfill` (~164 KB unminified) behind `globalThis.Temporal ?? bundled`, so it uses the runtime's own Temporal once there is one. Node 22+. Young project, small user base. | pg-boss 12.32.0 evaluates every rule schedule with it, depending on it at `^2.2.5`. |
...
- **Every backend wants a string, so the engine needs a parser either way.** BullMQ's `repeatStrategy` receives `opts.pattern` and parses it on every fire, and on pg-boss the expression is the schedule row's own column. A hand-written engine would mean writing an RFC 5545 parser as well, or smuggling JSON through fields that other tooling reads as a recurrence.
- **On pg-boss, the engine's dependency footprint is paid twice or once.** pg-boss carries `rrule-temporal` from 12.31.0 whether we use it or not, so a service on that adapter installs it regardless. Choosing it in core adds nothing to that service's install; choosing `rrule-rust` adds a native module and a platform matrix on top of a pure-JS engine already in the tree.
...
Recommendation for decision 1: **`rrule-temporal` behind a library-agnostic API**, declared on the same `^2.2.5` range pg-boss declares so the library and the backend cannot drift onto two copies of the same engine, with the fixture corpus as the swap guarantee.
...
| Adapters | One string for all transports: BullMQ's `pattern` and the pg-boss schedule row's own expression column, which takes RRULE text verbatim from 12.31.0. | Every transport needs the encoder anyway, so the same string exists, just derived. |
...
**The codec belongs in core, not in an adapter.** RRULE text is not a BullMQ artefact: BullMQ's `pattern` and the pg-boss schedule row carry the same string, and both products need the encoder to write a schedule and the decoder to render one. So core owns `encode`, `decode`, the profile validator and `nextOccurrence`, while an adapter only decides where the string travels and how the next fire is triggered. Nothing about the model differs per backend, which is part of what keeps the two adapters swappable.
...
type FireContext = {
  ...
  firedAt: Date                    // the occurrence; resolved from the delivery on pg-boss
  db?: TransactionRunner           // pg-boss only: the transaction the fired job is completed in
}
...
**`onFire` is a transaction on pg-boss, and delivery is at-least-once on both.** On the pg-boss adapter `onFire` runs inside `work({ transactional: true })`, so `ctx.db` is the transaction the fired job is completed in: the handler's domain rows, anything it enqueues through it, and the completion settle together or not at all. A handler that throws, blows its `expireInSeconds` or loses its claim to an operator or a supervisor rolls back whole and is retried with nothing committed, so the outcome where the work committed and the acknowledgement did not is gone there. Two things it does not close: side effects outside the database, which is every HTTP call a handler makes, and an occurrence that arrives as two jobs rather than as two attempts at one. So `ctx.fireId` stays exactly where it was, the same on every delivery of one occurrence and the thing a handler writes its own idempotency row against; the library does not dedupe domain work on its behalf. On BullMQ `ctx.db` is absent and a fire is at-least-once in full. `onDispatchFailed` runs when a delivery exhausts its retries, which is where a service marks the occurrence failed in its own tracking.
...
- **Transactional fires.** From 12.32.0 the fire handler runs inside the transaction its job is completed in, so what an occurrence did in the database and the record that it was handled cannot come apart. A handler that fails leaves neither behind and is retried on the queue's ordinary terms.
...
- A floor of pg-boss 12.32.0, and the rule grammar in it gates deployment-wide: an instance still on 12.30.0 reads a stored rule as cron, warns `invalid_schedule` and skips the row, so the bump has to reach every instance before the first rule schedule is written. Transactional workers add no schema change and no sequencing of their own.
- A pool connection per fire in flight, held for the handler's duration. `max` has to clear the transactional concurrency the adapter registers with room left for fetches, failures and maintenance, and pg-boss warns `transactional_pool_headroom` at `work()` time when it does not.
- An open transaction holds its snapshot and holds vacuum off every table in the database while the handler runs. That is a second reason for "enqueue domain work rather than running it inline", alongside the 24-hour `active` cap.
...
1. `schedule(job)` passes each rule through as the expression, once per rule in the recurrence. There is no format argument: `schedule()` reads the expression, decides it is a rule, and records that in the row's `kind`. `owner` sits at the top level of the schedule data so `list` can filter on it, and the rule sits beside it so the fire handler can resolve its own occurrence without a query.
   ...
   ```
2. pg-boss evaluates the rule on each pass and forwards a job for each minute the 60-second window holds an occurrence in, plus one for the gap since the last pass when the schedule asks for `once`. The worker is registered `work(queue, { transactional: true, includeMetadata: true }, (jobs, tx) => ...)` and invokes `onFire` with `ctx.db` bound to `tx`. `batchSize` stays at its default of 1, which keeps one occurrence to one transaction rather than letting one bad fire roll back its neighbours.
...
Reading the delivery time needs `includeMetadata` on the worker, which the adapter sets alongside `transactional`. One upstream follow-up would retire the walk back: occurrence identity on the fired job, so a handler reads the occurrence rather than deriving it. Transactional workers do not retire it, and they do not retire the idempotency row either, since what they make atomic is one job's handler and one job's completion, not the mapping from an occurrence to a job.
...
- No transactional writes and no transactional fires: registering a schedule cannot be committed with the domain row it belongs to, nothing the fire handler writes or enqueues can be committed with the job's acknowledgement, so `ctx.db` is absent here, and a fanned-out group's writes cannot be atomic either. That is inherent to Redis plus Postgres, so it belongs in the capability matrix rather than in the adapter's backlog, and it is why `fireId` idempotency stays a contract core promises on both adapters rather than something the pg-boss transaction lets a caller drop.
...
- **The option surface is what both adapters can honour.** An option only one backend can implement goes in the capability matrix and is rejected at call time by the other, rather than silently behaving differently. `catchUp` is the one that has to be written down, and it is `skip` and `once` because that is what upstream settled on and what BullMQ can match:
  ...
  | `catchUp` | pg-boss | BullMQ |
  | --- | --- | --- |
  | `skip` | `missed: 'skip'`, the upstream default: an occurrence older than the 60-second window is never sent | the adapter skips `onFire` on the same rule; the scheduler keeps running |
  | `once` | `missed: 'once'`: the pass sends one job for the most recent occurrence in the gap since the last pass | the scheduler's own behaviour: one overdue occurrence fires on resume |
...
- **Capability matrix, published with the package.** Transactional writes and transactional fires are both pg-boss only, and the presence of `ctx.db` is how a caller sees which it has. Fire precision differs by an order of magnitude and in kind: BullMQ fires at the computed millisecond, pg-boss files a job under the minute the occurrence falls in. Batching semantics, retention and what the read path returns are all matrix rows, filled from the conformance suite rather than from prose.
...
- `ScheduledJob.recurrence` is the canonical rules the caller passed in (or that `encode(form)` produced from their form). pg-boss carries each in the schedule row's own expression column, BullMQ in the scheduler template data and as `pattern`. It is the same string on every transport, so the read path returns the same thing regardless of backend, and `decode` turns it back into a form for rendering.
...
| --- | --- | --- |
| pg-boss | The schedule row's own expression column | Meets the requirement on its own, and from 12.31.0 the row also carries `kind` and `lastJobId`, so "which format" and "what did it last run" are readable without a join. Products keep their own row for listing and history. |
...
`list` filters on the top-level `owner` key in the schedule data. On pg-boss that filter runs in the adapter, because `getSchedules(name)` returns every row for the queue with no filter, limit or cursor. Either way it is adequate for admin tooling, and customer-facing lists come from the product tables.
...
Checked against released 12.32.0, which is [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898) plus [timgit/pg-boss#890](https://github.com/timgit/pg-boss/pull/890). Only what shapes the design is listed.
...
| `unschedule(name, key)` | Removes one rule's row, so a group is one call per key inside the caller's transaction. |
| `work(name, { transactional: true, includeMetadata: true }, (jobs, tx) => ...)` (12.32.0) | The fire path. pg-boss opens a transaction around the handler and completes the job in it, so `ctx.db` is `tx` and the handler's database work settles with the acknowledgement. `batchSize` stays at 1, so one occurrence is one transaction. It needs a `db` pg-boss can open a transaction on, which the built-in pool is; a service handing pg-boss a custom `db` has to implement `beginTransaction()` on it or `work()` throws at registration rather than on the first fire. |
| `findJobs(name, { id })` | Resolves `lastJobId` to the run it names, for a "last run" column and for ops tooling. |
| `{ db: fromDrizzle(tx, sql) }` on every write, from pg-boss's own adapter exports | Registering or cancelling a schedule commits with the service's domain write, and a fanned-out group commits whole. Covers both drizzle drivers in use: postgres-js in maestro, node-postgres in CTENG. It wraps a transaction the caller already opened, so it serves the write path only; the fire path's transaction is pg-boss's own. |
| `keep_until = start_after + retentionSeconds` | A month-ahead occurrence is not garbage-collected before it fires. |

Five constraints to design around:

- `expireInSeconds` caps `active` time at 24 hours, and a transactional fire holds a pool connection and a snapshot for as long as its handler runs, so the fire handler enqueues domain work instead of running it inline. `transactionTimeoutSeconds` is the database-side backstop for a process that dies under an open transaction, and it defaults to `expireInSeconds` plus five seconds, so setting the expiry sets the backstop with it.
- A SQL error the fire handler catches leaves its transaction aborted, and Postgres then rejects the completion too, so the job fails although the handler returned. The handler either lets such an error propagate or isolates the statement behind a `SAVEPOINT` of its own.
...
## Upstream: what 12.31.0 and 12.32.0 contain

### 12.31.0: rule schedules

[timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898) merged on 2026-09-10 (27 files, migration v41, tests and docs) and shipped the same day as 12.31.0. What our deployments run today is 12.30.0: `schedule()` takes a cron string and nothing else, the timekeeper evaluates `cron-parser`'s `prev()` against database time every 30 seconds, sends when the occurrence is inside the last 60 seconds, and deduplicates across instances with `singletonSeconds: 60`.

What the release changes:

- **RRULE is a built-in expression format** rather than a plugin point. A rule goes in the same string argument a cron expression goes in, and an expression carrying a `FREQ=` part or opening with an iCalendar property is read as one. Occurrences come from `rrule-temporal`, which pg-boss now depends on at `^2.2.5`.
...
### 12.32.0: transactional workers

[timgit/pg-boss#890](https://github.com/timgit/pg-boss/pull/890) merged and shipped on 2026-09-14, and 12.32.0 is the registry's `latest`. Schema 41 is unchanged from 12.31.0, so nothing migrates.

- **`work(name, { transactional: true }, (jobs, tx) => ...)`.** pg-boss opens a transaction around the handler and completes the job inside it, handing the handler that transaction as a second argument. Anything written through it, including a `send()` given `{ db: tx }`, commits with the completion or not at all. A throw rolls the pair back and the job is then failed on a pooled connection, so retry counts, retry delays and dead lettering stay the ordinary code path rather than a second copy of it.
- **The job is claimed before the transaction opens**, so it is `active` while the handler runs and every supervision path sees it: `expireInSeconds` bounds it, heartbeats refresh it, group limits count it, and a process that dies mid-handler leaves a row the supervisor reclaims. Fetching, batching and concurrency sit outside the transaction and are unchanged.
- **The commit needs the claim the handler started with.** If an expiry, a heartbeat the database stopped seeing, an operator's `cancel()` or another instance's supervisor takes the job away mid-handler, the transaction rolls back rather than committing side effects under a job that is about to run again. A handler that settles its own jobs through `{ db: tx }` is recognised and unaffected; one that writes the job table directly reads as a lost claim.
- **`transactionTimeoutSeconds`** is the database-side bound, defaulting to `expireInSeconds` plus the five seconds pg-boss allows its own rollback. It is not the timer that ends a slow handler, which is `expireInSeconds` and fires first by design; it covers the process failing under an open transaction, which otherwise advertises `backend_xmin` and holds vacuum off every table in the database. It is applied as `transaction_timeout` where the server has it (PostgreSQL 17 and newer) and `idle_in_transaction_session_timeout` otherwise, and it never widens a bound the connection already carries.
- **`beginTransaction()` joins `IDatabase`** as an optional capability beside `listen`, returning a handle of `db`, `commit()` and `rollback()`, with `withTransaction()` now a wrapper over it. The built-in pool implements it. `work({ transactional: true })` throws at registration on an adapter without it, so a service that hands pg-boss a custom `db` learns at startup rather than on the first batch.
- **`perJobResults` and `transactional` are rejected together**, since one transaction has one outcome, and TypeScript picks the two-argument handler off a literal `transactional: true`.
- **Two new warnings**, both emitted as events and never written to the `warning` table: `transactional_pool_headroom` when a transactional worker is registered on a pool with nothing to spare, and `transaction_timeout_probe` when the server will not say which timeout parameter it recognises, which leaves handlers running with no database-side bound.

Three costs, and the adapter lives with all of them: a pool connection per handler in flight, a snapshot that holds vacuum back for the handler's duration, and a SQL error the handler catches leaving the transaction aborted so the completion fails with it. All three point the same way, at a fire handler that enqueues and returns.

What the adapter becomes on top of it: `schedule` and `reschedule` are `boss.schedule` per rule with `missed` in the options, `unschedule` is `boss.unschedule` per key, `get` is `boss.getSchedule` plus `previewSchedule`, and `onFire` is `boss.work` with `transactional` and `includeMetadata` set. The schedule row is the registry, so there is no `singletonKey` bookkeeping and no payload scraping, and `reconcile` is a consistency check against the domain table and nothing else. Ours to keep: the codec and profile, the fan-out and grouping, occurrence resolution on the fire path, and `previewOccurrences` over a multi-rule recurrence.

### Rolling out the upgrade

The bump is to 12.32.0. What gates deployment-wide rather than per-process is the rule grammar, which has to reach every instance of a deployment before the first rule schedule is written there:
...
- 12.32.0 asks for no sequencing of its own. It carries no schema change, and a worker registered without `transactional` behaves exactly as it did, so the two releases roll out as one bump.

A rule schedule fires only while at least one upgraded instance runs the timekeeper, so the first rule is written once the bump has rolled out everywhere, and the adapter asserts the version it needs at startup rather than detecting it and degrading quietly.

### Smaller upstream follow-ups

`getSchedule`, `previewSchedule`, `lastJobId` and the `missed` policy shipped in [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898), and the transactional worker in [timgit/pg-boss#890](https://github.com/timgit/pg-boss/pull/890). What is left, in the order it matters to us:
...
- The pg-boss adapter is built last (step 5 of the migration outline). Both releases it rests on are out, so the adapter is a mapping onto `boss.schedule` and `boss.work` and there is no fallback strategy to write.
- Recurrence is pg-boss's own, gated on the whole deployment running 12.31.0 or newer, with `>=12.32.0` the range the adapter declares so the fire path has its transaction. One adapter covers both expression formats, which differ only in what goes in the expression argument.
- Nothing in steps 1 to 4 touches pg-boss: they are the engine, the codec, the facade and the BullMQ adapter. Step 1 does depend on the engine choice, and choosing the engine pg-boss uses is what makes step 5 a mapping rather than a reconciliation, so it is settled first even though its payoff arrives last.
...
- `@lokalise/scheduling-common/pg-boss`: schedule registration and reads, queue setup (`short` policy), transactional worker registration, transactional send helper, the fan-out and grouping of a multi-rule recurrence. `pg-boss` as an optional peer dependency, `>=12.32.0`.
...
Dependencies of core: `zod`, `rrule-temporal` on the same `^2.2.5` range pg-boss declares, `cron-parser` for legacy cron patterns. No dependency on `background-jobs-common`, `ioredis`, `bullmq` or `pg-boss` in core. Node 22.12 or newer, which is pg-boss's own floor and `rrule-temporal`'s. The conformance suite runs against both entry points, so the package's docker-compose needs a Postgres container alongside the Redis one `background-jobs-common` already uses.

Matching pg-boss's range on `rrule-temporal` is a deliberate coupling, not caution about a young dependency. Two copies of the engine in one install means the library previewing occurrences one way and pg-boss firing them another, which is the exact failure the engine choice exists to prevent. Identical ranges resolve to one copy, the lockfile is what makes a bump deliberate, and the conformance suite is what says it may happen.
...
5. **pg-boss adapter.** `boss.schedule` per rule with `missed` in the options and `boss.work` with `transactional` on the fire path, gated on the bump to 12.32.0 having rolled out everywhere, per "Rolling out the upgrade". Full test suite (upsert reschedule, group fan-out and tail deletion, retention, transactional send, transactional fire including a rollback under a lost claim and one under a SQL error the handler caught, reconcile, both `missed` policies in both expression formats, occurrence resolution on the fire path). The conformance suite starts running against both adapters here. The remaining upstream proposals get filed alongside, occurrence identity on a fired job first.
6. **CTENG on the facade.** Replace `ScheduleRunnerJobProcessor` and `ScheduleNextRunRecomputeJobProcessor` with the facade: on save, `schedule` or `reschedule`; the `onFire` handler runs today's `executeScheduleInTransaction` logic and `schedule_execution` tracking unchanged, now with `ctx.db` as that transaction, so the `schedule_execution` row and the fired job's completion settle together. The Autopilot call stays outside it, so that dispatch is at-least-once and still keyed on `fireId`. `next_run_at` becomes a cached display value populated from `get()`, or is dropped, and the daily recompute job goes with it, since nothing caches a fire time any more. Fire precision improves from tick granularity to per-occurrence, within the one-job-a-minute ceiling. Every schedule registers `catchUp: 'once'`, which is what keeps CTENG's current "fire the missed occurrence once" behaviour intact: the pg-boss default is `skip`, and taking it would silently turn a deploy window into skipped runs. `schedule_execution` stays domain tracking and is not an input to catch-up. On the pg-boss adapter, since CTENG runs no Redis for this and the schedule stays in the database it already backs up; that is CTENG's call to make, and the BullMQ adapter would work too at the cost of new infrastructure.
...
- **Engine choice.** Recommended: `rrule-temporal`, the engine pg-boss uses from 12.31.0, on the same `^2.2.5` range pg-boss declares (decision 1). It is pure JS with one types-only dependency and needs no polyfill, and choosing it means the library and the backend cannot disagree about when a rule fires. What it costs is maestro giving up `rrule-rust`, which has more of our own production mileage than the replacement does, and a young dependency at the centre of two products' scheduling. The shared range, the fixture corpus and step 1's side-by-side run are the three things that make that acceptable, and none of them is optional.
...
- **pg-boss is new infrastructure for whoever adopts it.** A `pgboss` schema in that service's database, automatic migrations on `start()` (pin the version and disable `migrate` on all but one instance during upgrades), a `pg` pool alongside the existing driver and sized above the transactional fire concurrency since each fire in flight holds a connection for its handler's duration, and poll-interval fire precision. Runbooks change with the backend: on pg-boss, CUJ-002's recovery step becomes "check pending jobs in `pgboss`, run reconcile". This is a cost of choosing the backend, not of adopting the library.
...
- **A dead schedule is silent without reconcile.** `reconcile` ships in the first release and both services run it periodically. What counts as missing differs per backend, and the wrong predicate either hides the outage or pages on healthy schedules: on BullMQ a live schedule has a job scheduler with a `next`, and on pg-boss it has only a schedule row whose expression still yields a future occurrence, with no pending job at all between fires and nothing on the row to read. `list()` is what normalises that, computing liveness through `previewSchedule` on the pg-boss path, so the alert stays one diff against the domain table.
...
- **A transactional fire constrains what a fire handler may do.** Its transaction is open for the handler's duration, holding a pool connection and holding vacuum back, and a SQL error the handler catches itself aborts the transaction so the completion fails with it. Both point at a handler that enqueues domain work and returns in milliseconds, which is what the 24-hour `active` cap already asked for, and both are conformance-suite cases rather than prose. A handler's HTTP calls are outside the transaction whatever it does, so `fireId` idempotency stays part of the contract on both adapters and the pg-boss transaction is not a licence to drop it.
- **The floor is 12.32.0, and the rule grammar in it gates deployment-wide.** [timgit/pg-boss#898](https://github.com/timgit/pg-boss/pull/898) and [timgit/pg-boss#890](https://github.com/timgit/pg-boss/pull/890) are both released, so the expression grammar, the `kind` semantics, the `previewSchedule` signature, the `missed` policy and the transactional worker contract are settled rather than provisional, and this document is written against shipped code. What is left is the rollout: an instance still on 12.30.0 reads a stored rule as a cron expression, warns `invalid_schedule` and skips the row, so the bump reaches every instance of a deployment before its first rule schedule is written. Transactional workers need no such sequencing, carrying no schema change and leaving a worker without the option exactly as it was. That is step 5's first task, per "Rolling out the upgrade".
